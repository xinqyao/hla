# read dG energies from both complex and ligand-only simulations
lines.comp <- readLines('production/results.txt')
lines.lig <- readLines('ligand/production/results.txt')
...
dg <- -dg1 + dg2 + dg3
dg.err <- sqrt(dg1.err^2 + dg2.err^2)
